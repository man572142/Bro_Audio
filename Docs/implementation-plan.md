...
| `Assets/BroAudio/Runtime/Utility/ClipSelection/LocalizationClipStrategy.cs` | **New file.** Implements `IClipSelectionStrategy`. Accepts table/entry/name via `Inject()`. `SelectClip()` validates table and entry are set, resolves the `AudioClip` via `WaitForCompletion()`, finds the matching `BroAudioClip` row by locale, and returns a `LocalizedBroAudioClipWrapper`. Falls back to a wrapper with null `BroAudioClip` (default playback properties) if no locale row matches. Returns `null` if the resolved `AudioClip` is null. `Reset()` is a no-op (stateless). **Note:** `WaitForCompletion()` can hitch on first play if the asset is not preloaded. The preload API (`BroAudio.PreloadLocalizationAssets`) is deferred — see spec §7. |
| `Assets/BroAudio/Runtime/DataStruct/Core/AudioEntity.Localization.cs` | Replace the Phase 2 stub with the `Localization` case in `PickNewClip()`: call `EnsureClipSelectionStrategy<LocalizationClipStrategy>()`, inject table/entry/name, and delegate to `strategy.SelectClip()`. All resolution and null-handling logic lives in the strategy. |
...
| **Each row** | Rows are auto-generated by `SyncClipsWithLocales()` to match `LocalizationSettings.AvailableLocales` — no Add/Remove buttons. Locale label (read-only, from `BroAudioClip.Locale`). `AudioClip` ObjectField — reads/writes through the Localization editor API to the Asset Table (never writes to `BroAudioClip.AudioClip`). Play preview button (resolves from table). Volume slider (→ `BroAudioClip.Volume`). Volume values are preserved across re-syncs via a dictionary keyed on locale code. |

...
namespace Ami.BroAudio.Editor
{
    ...
    {
        private class PreviewHelper : IEditorPreviewable
        {
            private SerializedProperty _volProp;

            public PreviewHelper(SerializedProperty volProp)
            {
                _volProp = volProp;
            }

            public float Volume => _volProp.floatValue;
        ...
        private SerializedProperty _volumeProp = null;
        private PreviewHelper _previewHelper = null;
        ...
        private void OnEnable()
        {
            ...
            _volumeProp = _serializedObject.FindProperty(nameof(_volume));
            _previewHelper = new PreviewHelper(_volumeProp);
        ...
        private void OnGUI()
        {
            ...
            if(_volumeProp != null && _transport != null)
            {
                EditorPlayAudioClip.Instance.UpdatePreviewClipValues(_volumeProp.floatValue, AudioConstant.DefaultPitch, _transport);
            }
        ...
        private void DrawVolumeSlider()
        {
            ...
            _volumeProp.floatValue = BroEditorUtility.DrawVolumeSlider(rect, new GUIContent("Volume"), _volumeProp.floatValue, _isVolumeSnapped, () => _isVolumeSnapped = !_isVolumeSnapped);
        }
        ...
        private void DrawClipPreview(float height, float volume, out Rect previewRect)
        {
            ...
            _clipPropHelper.DrawClipPreview(previewRect, _transport, TargetClip, _previewHelper, TargetClip.name); // don't worry about any duplicate path, because there will only one clip in editing                                                                                                                                         //DrawEmptyLine(GetLineCountByPixels(height));
        }
            ...
            if (GUILayout.Button(playButtonContent, width, height))
            {
                ...
                else if(_audioClipProp.objectReferenceValue is AudioClip clip)
                {
                    ...
                    else
                    {
                        ...
                        info = new PreviewClipInfo(clip.length);
                    }
